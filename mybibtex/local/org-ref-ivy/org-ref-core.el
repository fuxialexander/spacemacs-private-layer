...
(defcustom org-ref-bibliography-notes
  nil
  "Filename for all your notes about an entry in the default bibliography.
...
(defcustom org-ref-notes-directory
  nil
  "Directory for all your notes about an entry in the default bibliography.
    ...
    (let* ((bibfile)
      ...
      (let (key-beginning key-end)
        ...
        (if (file-exists-p bibfile)
          ...
          (catch 'done
            (cl-loop for path in (split-string (getenv "BIBINPUTS") ":")
                     do
                     (when (file-exists-p
    ...
    (save-restriction
      ...
      (let ((matches '()))
        (while (re-search-forward "^\\( \\)*#\\+name:\\s-+\\(.*\\)" nil t)
          (cl-pushnew (match-string 1) matches))
        matches))))
    ...
    (save-excursion
      (save-restriction
        ...
        (while (re-search-forward
                ...
                nil t)
          (cl-loop for bibfile in (mapcar 'org-ref-strip-string
                   ...
                   (cond
                    ...
                    ((getenv "BIBINPUTS")
                     (cl-loop for bibdir in (split-string (getenv "BIBINPUTS") ":")
                              do
